...
def compute_epoch_cm(trainer) -> None:
    with suppress(AttributeError):
        trainer.state.train_cm = sum(
            (state["confusion_matrix"] for state in trainer.state.train.minibatch_stats)
        )

    with suppress(AttributeError):
        trainer.state.eval_cm = sum(
            (state["confusion_matrix"] for state in trainer.state.eval.minibatch_stats)
        )

    with suppress(AttributeError):
        trainer.state.test_cm = sum(
            (state["confusion_matrix"] for state in trainer.state.test.minibatch_stats)
        )
